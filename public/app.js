...
function mediaMarkup(){
 ...
 <div class="field"><label>Thông tin bổ sung</label><input id="analysis-context" maxlength="300" placeholder="Ví dụ: Nho Hạ Đen trồng chậu, dịch vụ vệ sinh sofa..."><small>Giúp AI nhận đúng sản phẩm, giống, dịch vụ hoặc nội dung khó nhận biết chỉ qua hình ảnh.</small></div>
...
async function runAnalysis(){
 ...
 try{
  ...
  const result=await api('analysis',{mode:'scene_frames',duration:v.duration,context:($('#analysis-context')?.value||'').trim(),frames:detected.frames,sceneMeta:{scanned:detected.scanned,interval:detected.interval,threshold:detected.threshold},consent:true});

...
function scriptMarkup(){
 ...
 '<div id="style-prompt-viewer" class="notice" hidden><div class="row between"><strong id="style-prompt-title">Prompt phong cách</strong><button type="button" id="close-style-prompt" class="small">Đóng</button></div><div id="style-prompt-text" class="prompt-preview"></div><div id="style-prompt-actions" class="style-prompt-actions" hidden><button type="button" id="edit-style-from-prompt" class="small primary" data-edit-script-style="">Sửa phong cách</button><button type="button" id="delete-style-from-prompt" class="small danger" data-delete-script-style="">Xóa phong cách</button></div></div>'+
 '<details class="custom-style-builder"><summary id="custom-style-summary">+ Tạo phong cách riêng</summary><input id="custom-style-id" type="hidden"><div class="field"><label>Tên phong cách</label><input id="custom-style-name" maxlength="80" placeholder="Ví dụ: Review chân thật"></div><div class="field"><label>Prompt</label><textarea id="custom-style-prompt" rows="6" maxlength="6000" placeholder="Mô tả cách viết, giọng điệu, cấu trúc mở đầu, cách kết thúc..."></textarea></div><div class="row"><button type="button" id="create-script-style" class="primary">Lưu phong cách</button><button type="button" id="cancel-edit-script-style" class="small" hidden>Hủy sửa</button></div><p id="custom-style-status" class="status-line"></p></details>'+
...
function showStylePrompt(style){
 if(!style)return;
 const title=$('#style-prompt-title'),textBox=$('#style-prompt-text'),viewer=$('#style-prompt-viewer'),actions=$('#style-prompt-actions'),edit=$('#edit-style-from-prompt'),remove=$('#delete-style-from-prompt');
 if(!title||!textBox||!viewer)return;
 const isCustom=style.kind==='custom'||(S.scriptStyles?.custom||[]).some(item=>item.id===style.id);
 title.textContent=style.name;textBox.textContent=style.prompt;viewer.dataset.styleId=style.id;viewer.hidden=false;
 if(actions)actions.hidden=!isCustom;
 if(edit){edit.hidden=!isCustom;edit.dataset.editScriptStyle=isCustom?style.id:'';edit.setAttribute('aria-label',`Sửa ${style.name}`)}
 if(remove){remove.hidden=!isCustom;remove.dataset.deleteScriptStyle=isCustom?style.id:'';remove.setAttribute('aria-label',`Xóa ${style.name}`)}
}
...
function bindScriptEvents(){
 ...
 const resetCustomStyleForm=()=>{const id=$('#custom-style-id'),name=$('#custom-style-name'),prompt=$('#custom-style-prompt'),save=$('#create-script-style'),cancel=$('#cancel-edit-script-style'),summary=$('#custom-style-summary'),status=$('#custom-style-status');if(id)id.value='';if(name)name.value='';if(prompt)prompt.value='';if(save)save.textContent='Lưu phong cách';if(cancel)cancel.hidden=true;if(summary)summary.textContent='+ Tạo phong cách riêng';if(status)status.textContent=''};
 const editCustomStyle=styleId=>{const style=(S.scriptStyles.custom||[]).find(x=>x.id===styleId);if(!style)return toast('Không tìm thấy phong cách riêng.',true);const details=$('.custom-style-builder'),id=$('#custom-style-id'),name=$('#custom-style-name'),prompt=$('#custom-style-prompt'),save=$('#create-script-style'),cancel=$('#cancel-edit-script-style'),summary=$('#custom-style-summary'),status=$('#custom-style-status');if(!details||!id||!name||!prompt||!save||!cancel)return;details.open=true;id.value=style.id;name.value=style.name;prompt.value=style.prompt;save.textContent='Cập nhật phong cách';cancel.hidden=false;if(summary)summary.textContent='Chỉnh sửa phong cách riêng';if(status)status.textContent='';name.focus()};
 $$('[data-edit-script-style]').forEach(b=>b.onclick=()=>editCustomStyle(b.dataset.editScriptStyle));
...
function renderAdmin(){
 ...
 const writing=S.adminState.scriptWriting&&typeof S.adminState.scriptWriting==='object'?S.adminState.scriptWriting:{};
 const writingProvider=$('#admin-script-writing-provider');
 const writingNotice=$('#admin-script-writing-notice');
 const writingModel=$('#admin-script-writing-model');
 const writingStatus=$('#admin-script-writing-status');
 if(writingProvider)writingProvider.value=['deepseek','openai'].includes(writing.provider)?writing.provider:'deepseek';
 if(writingNotice){const label=writing.provider==='openai'?'ChatGPT (OpenAI API)':'DeepSeek';const configured=writing.provider==='openai'?writing.openaiConfigured:writing.deepseekConfigured;writingNotice.className='notice '+(configured?'success':'warning');writingNotice.textContent=`Đang dùng ${label}. ${configured?'API key đã sẵn sàng.':'API key chưa được cấu hình.'}`}
 if(writingModel)writingModel.textContent=writing.model?`Model hiện tại: ${writing.model}`:'';
 if(writingStatus)writingStatus.textContent=S.adminState.redis?'Lựa chọn được lưu dùng chung cho toàn hệ thống.':'Cần Upstash Redis để lưu lựa chọn sau khi triển khai.';
 const saveWriting=$('#admin-save-script-writing-provider');
 if(saveWriting)saveWriting.onclick=()=>busy(saveWriting,async()=>{const provider=writingProvider?.value;const config=await api('admin-save-script-writing-provider',{provider});S.adminState.scriptWriting=config;renderAdmin();toast(`Đã lưu chế độ viết bài: ${provider==='openai'?'ChatGPT':'DeepSeek'}.`) });

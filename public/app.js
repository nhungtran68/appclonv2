...
function mediaMarkup(){
 ...
 <div class="field"><label>Thông tin sản phẩm / dịch vụ <span class="muted">(không bắt buộc)</span></label><input id="analysis-subject-hint" maxlength="300" placeholder="Ví dụ: Nho giống Hạ Đen, máy lọc nước RO, dịch vụ spa trị mụn..."><small>Nhập ngắn gọn để AI nhận diện chính xác hơn khi hình ảnh khó xác định.</small></div>
...
function scriptMarkup(){
 ...
 const customList=custom.length?'<div class="custom-style-list">'+custom.map(style=>'<div class="custom-style-item"><span>'+esc(style.name)+'</span><div class="row"><button type="button" class="small" data-edit-script-style="'+esc(style.id)+'">Sửa</button><button type="button" class="small danger" data-delete-script-style="'+esc(style.id)+'">Xóa</button></div></div>').join('')+'</div>':'';
...
function voiceMarkup(){
 ...
 return `<div class="hero"><div><span class="eyebrow">IBEE AIVOICE</span><h1>Chọn <span class="hero-accent">giọng phù hợp.</span></h1><p>Dùng giọng riêng được admin gán hoặc chọn một giọng Trend dùng chung.</p></div><span class="pill green">IBEE</span></div><div class="grid2">
 ...
 <section class="panel"><div class="panel-head"><h2>Audio kết quả</h2><span class="pill">MP3</span></div><div id="audio-output" hidden class="audio-output"><audio id="tts-preview" controls></audio><div class="row between"><span id="audio-duration" class="pill green"></span><button id="download-audio" class="small">${icon('down')} Tải MP3</button></div></div><div class="notice">Danh sách giọng chỉ hiển thị tên. Mã giọng được giữ ở máy chủ và không hiển thị cho tài khoản sử dụng.</div></section></div>`;
}
function renderVoicePage(){const page=$('#page-voice');if(!page)return;const prev=$('#voice-text')?.value||'';page.innerHTML=voiceMarkup();if($('#voice-text'))$('#voice-text').value=prev;bindVoiceEvents();updateCounts()}
...
function selectVideo(id){
 ...
 if($('#analysis-subject-hint'))$('#analysis-subject-hint').value=v?.analysisSubjectHint||'';
...
async function runAnalysis(){
 ...
 try{
  ...
  const subjectHint=($('#analysis-subject-hint')?.value||'').trim();
  const result=await api('analysis',{mode:'scene_frames',duration:v.duration,subjectHint,frames:detected.frames,sceneMeta:{scanned:detected.scanned,interval:detected.interval,threshold:detected.threshold},consent:true});
  v.analysisSubjectHint=subjectHint;
